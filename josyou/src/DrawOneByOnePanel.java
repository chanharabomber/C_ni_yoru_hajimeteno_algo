 ...
 *
 *  @version $Revision: 1.13 $, $Date: 2003/03/19 09:08:34 $
 */
...
public abstract class DrawOneByOnePanel extends JPanel
                                        implements Runnable {
    ...
    public DrawOneByOnePanel(int width, int height, int space) {
        ...
        setPreferredSize(new Dimension(width  + border * 2,
                                       height + border * 2));
    ...
    abstract public void drawSlowly(Graphics2D g);  // サブクラスで実装する

    private int wait = 0;  // 累積待ち時間
    static final int WAIT_MIN = 100;

    /** 実画面に表示して時間待ち */
    public boolean sleep() {  return sleep(WAIT_MIN);  }
